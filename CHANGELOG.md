...
Initial release. Stable .NET API for the
[3ncr.org v1](https://3ncr.org/1/) encryption envelope (AES-256-GCM,
12-byte random IV, 16-byte GCM tag). Targets `net8.0`.
...
- `ThreeNcr.TokenCryptException` — single exception type for decryption
  failures (bad tag, truncated input, malformed base64).

### Notes

- Cross-verified against the canonical v1 test vectors shared with the Go,
  Node.js, PHP, Python, Rust, and Java reference implementations.
- Per 3ncr.org's new-language convention, the legacy PBKDF2-SHA3 KDF is
  intentionally omitted — there is no pre-existing .NET data to decrypt.
  Callers needing it can derive the key with a PBKDF2-SHA3-256 implementation
  (e.g. BouncyCastle's `Pkcs5S2ParametersGenerator` with `Sha3Digest(256)`)
  and pass the result to `FromRawKey`.

[1.0.0]: https://github.com/3ncr/tokencrypt-csharp/releases/tag/v1.0.0
